		...
		// -------------------------------------------------------------------------
			...
			// -------------------------------------------------------------------------
				...
				stage('Authorize DevHub And Create Scratch Org') {
					rc = sh returnStatus:true , script : "sf org login jwt --instance-url ${SFDC_HOST} --client-id ${SF_CONSUMER_KEY} --username ${HUB_ORG_USERNAME} --jwt-key-file ${server_key_file} --setdefaultdevhubusername"
					if (rc != 0) {
